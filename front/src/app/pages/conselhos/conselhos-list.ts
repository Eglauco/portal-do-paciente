...
import { Conselho } from './conselho.model';
import { ConselhoBuscaStore } from './conselho-busca.store';
import { ConselhoService } from './conselho.service';

export type PaginaItem = number | 'ellipsis';

@Component({
  selector: 'app-conselhos-list',
  imports: [RouterLink, RelatorioColunasModal, Ordenavel],
  templateUrl: './conselhos-list.html',
})
export class ConselhosList {
  private readonly service = inject(ConselhoService);
  private readonly router = inject(Router);
  private readonly store = inject(ConselhoBuscaStore);
  ...
  protected readonly tamanhos = ConselhoService.TAMANHOS;
  ...
  protected readonly conselhos = signal<Conselho[]>([]);
  ...
  protected readonly fimFaixa = computed(() => this.page() * this.size() + this.conselhos().length);
  ...
  private carregar(): void {
    ...
    this.service
      ...
      .subscribe({
      next: (pagina) => {
        this.conselhos.set(pagina.content);
  ...
  protected editar(conselho: Conselho): void {
    this.router.navigate(['/conselhos', conselho.id]);
  }
  ...
  protected confirmarExportacao(colunas: string[]): void {
    ...
    this.service
      ...
      .subscribe({
      next: (blob) => {
        this.baixar(blob, `conselhos.${formato}`);
