...
@Component({
  selector: 'app-motivo-falta-form',
  imports: [ReactiveFormsModule],
  templateUrl: './motivo-falta-form.html',
})
export class MotivoFaltaForm implements PodeSair {
  ...
  constructor() {
    ...
    if (idParam) {
      ...
      this.service.buscarPorId(id).subscribe({
        next: (motivo) => this.form.patchValue({ motivo: motivo.motivo, ativo: motivo.ativo }),
  ...
  protected invalido(campo: 'motivo'): boolean {
  ...
  protected salvar(event: Event): void {
    ...
    requisicao.subscribe({
      next: () => {
        ...
        this.toastr.success('Motivo salvo');
        this.router.navigate(['/motivos-falta']);
      },
      error: () => {
        this.salvando.set(false);
        this.toastr.error('Não foi possível salvar o motivo.');
      },
    });
  }
  ...
  protected async excluir(): Promise<void> {
    ...
    const confirmado = await this.confirmar('Deseja excluir o motivo?');
    if (!confirmado) return;
    this.excluindo.set(true);
    this.service.excluir(this.codigo()!).subscribe({
      next: () => {
        this.saidaAutorizada = true;
        this.toastr.success('Motivo excluído');
        this.router.navigate(['/motivos-falta']);
      },
      error: () => {
        this.excluindo.set(false);
        this.toastr.error('Não foi possível excluir o motivo.');
      },
    });
  }
