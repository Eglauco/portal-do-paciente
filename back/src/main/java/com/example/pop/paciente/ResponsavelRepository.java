...
public interface ResponsavelRepository extends JpaRepository<Responsavel, Long> {
    ...
    /** Este telefone é responsável DESTE paciente? (autorização do perfil dependente). */
    boolean existsByTelefoneAndPaciente_Id(String telefone, Long pacienteId);
    ...
    /** Pacientes (distintos) de quem este telefone é responsável — os perfis "dependentes". */
    @Query("select distinct r.paciente from Responsavel r where r.telefone = :telefone")
    List<Paciente> pacientesPorTelefoneDoResponsavel(@Param("telefone") String telefone);

    /** Telefones (distintos, não nulos) dos responsáveis deste paciente — contas que o acessam. */
    @Query("select distinct r.telefone from Responsavel r where r.paciente.id = :pacienteId and r.telefone is not null")
